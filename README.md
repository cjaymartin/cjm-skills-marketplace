...
| [`local-secrets`](skills/local-secrets/SKILL.md) | Keeps secrets in one gitignored file per environment, such as `.env.development` and `.env.production`, that you fill in by hand. The agent adds each key with the steps to get it, opens the file in VS Code at that line, and never sees the values. No more "paste your key here" commands. |
